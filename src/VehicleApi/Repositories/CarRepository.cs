...
{
    private static readonly List<Bike> Bikes = new()
    {
        new Bike()
        {
            BikeId = 1,
    ...
    public Task<Bike> AddAsync(Bike bike, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(bike);

        Bikes.Add(bike);
        bike.BikeId = Bikes.Max(x => x.BikeId) + 1;
        return Task.FromResult(bike);
    }

    public Task DeleteAsync(Bike bike, CancellationToken cancellationToken)
    {
    ...
    public Task<Bike?> GetAsync(int bikeId, CancellationToken cancellationToken)
    {
        var bike = Bikes.FirstOrDefault(x => x.BikeId == bikeId);
        return Task.FromResult(bike);
    }
}
